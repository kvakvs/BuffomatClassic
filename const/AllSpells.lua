...
local function bomSetupPriestSpells(spells, enchants)
  local priestOnly = { playerClass = "PRIEST" }

  BOM.Class.SpellDef:scanSpell(spells, 10938, -- Fortitude / Seelenstärke
  ...
  BOM.Class.SpellDef:scanSpell(spells, 10958, --Shadow Protection / Prayer of Shadow / Schattenschutz
          ...
          priestOnly)
  BOM.Class.SpellDef:scanSpell(spells, 6346, -- Fear Ward
  ...
  BOM.Class.SpellDef:scanSpell(spells, 19266, -- Touch of Weakness / Berührung der Schwäche
          { default      = true, isOwn = true,
            singleFamily = { 2652, 19261, 19262, 19264, 19265, 19266, -- Ranks 1-6
                             25461 } }, -- TBC: Rank 7
          priestOnly)
  BOM.Class.SpellDef:scanSpell(spells, 10952, -- Inner Fire / inneres Feuer
          { default      = true, isOwn = true,
            singleFamily = { 588, 7128, 602, 1006, 10951, 10952, -- Ranks 1-6
                             25431 } }, -- TBC: Rank 7
          priestOnly)
  BOM.Class.SpellDef:scanSpell(spells, 19312, -- Shadowguard
          { default      = true, isOwn = true,
            singleFamily = { 18137, 19308, 19309, 19310, 19311, 19312, -- Ranks 1-6
                             25477 } }, -- TBC: Rank 7
          priestOnly)
  BOM.Class.SpellDef:scanSpell(spells, 19293, -- Elune's Grace
          { default      = true, isOwn = true,
            singleFamily = { 2651, -- Rank 1 also TBC: The only rank
                             19289, 19291, 19292, 19293 } }, -- Ranks 2-5 (non-TBC)
          priestOnly)
  BOM.Class.SpellDef:scanSpell(spells, 15473, -- Shadow Form
          { default = false, isOwn = true },
          priestOnly)
  BOM.Class.SpellDef:scanSpell(spells, 20770, -- Resurrection / Auferstehung
...
---@param spells table<string, SpellDef>
---@param enchants table<string, table<number>>
local function bomSetupDruidSpells(spells, enchants)
  local druidOnly = { playerClass = "DRUID" }

  BOM.Class.SpellDef:scanSpell(spells, 9885, --Gift/Mark of the Wild | Gabe/Mal der Wildniss
  ...
  BOM.Class.SpellDef:scanSpell(spells, 9910, --Thorns | Dornen
          ...
          druidOnly)
  BOM.Class.SpellDef:scanSpell(spells, 16864, --Omen of Clarity
  ...
  BOM.Class.SpellDef:scanSpell(spells, 17329, -- Nature's Grasp | Griff der Natur
          { isOwn        = true, cancelForm = true, default = false,
            ...
            singleFamily = { 16689, 16810, 16811, 16812, 16813, 17329, -- Rank 1-6
                             27009 } }, -- TBC: Rank 7
          druidOnly)
  BOM.Class.SpellDef:scanSpell(spells, 33891, --TBC: Tree of life
  ...
  BOM.Class.SpellDef:scanSpell(spells, BOM.SpellId.Druid.TrackHumanoids, -- Track Humanoids (Cat Form)
...
---@param spells table<string, SpellDef>
---@param enchants table<string, table<number>>
local function bomSetupMageSpells(spells, enchants)
  ...
  if UnitLevel("player") >= 58 then
    ...
    BOM.Class.SpellDef:scanSpell(spells, BOM.SpellId.Mage.ConjureManaEmerald, -- Conjure Mana Stone (Max Rank)
...
---@param spells table<string, SpellDef>
---@param enchants table<string, table<number>>
local function bomSetupShamanSpells(spells, enchants)
  ...
  BOM.Class.SpellDef:scanSpell(spells, 16342, --Flametongue Weapon
  ...
  BOM.Class.SpellDef:scanSpell(spells, 16356, --Frostbrand Weapon
          { type         = "weapon", isOwn = true, isConsumable = false,
            default      = false, singleDuration = enchant_duration,
  ...
  BOM.Class.SpellDef:scanSpell(spells, 16316, --Rockbiter Weapon
          { type         = "weapon", isOwn = true, isConsumable = false,
            default      = false, singleDuration = enchant_duration,
  ...
  BOM.Class.SpellDef:scanSpell(spells, 16362, --Windfury Weapon
          { type         = "weapon", isOwn = true, isConsumable = false,
            default      = false, singleDuration = enchant_duration,
  ...
  BOM.Class.SpellDef:scanSpell(spells, 10432, -- Lightning Shield / Blitzschlagschild
  ...
  BOM.Class.SpellDef:scanSpell(spells, 33736, -- TBC: Water Shield 1, 2
          { isOwn = true, default = true, duration = duration, singleFamily = { 24398, 33736 } },
          { playerClass = "SHAMAN", isTBC = true })

  BOM.Class.SpellDef:scanSpell(spells, 20777, -- Ancestral Spirit / Auferstehung
...
---@param spells table<string, SpellDef>
---@param enchants table<string, table<number>>
local function bomSetupWarlockSpells(spells, enchants)
  local warlockOnly = { playerClass = "WARLOCK" }

  BOM.Class.SpellDef:scanSpell(spells, 5697, -- Unending Breath
          { default = false, singleDuration = DURATION_10M, targetClasses = BOM_ALL_CLASSES },
          warlockOnly)
  BOM.Class.SpellDef:scanSpell(spells, 11743, -- Detect Greater Invisibility | Große Unsichtbarkeit entdecken
          { default        = false, singleFamily = { 132, 2970, 11743 },
            singleDuration = DURATION_10M, targetClasses = BOM_ALL_CLASSES },
          warlockOnly)
  BOM.Class.SpellDef:scanSpell(spells, 28610, -- Shadow Ward / Schattenzauberschutz
          { isOwn = true, default = false, singleFamily = { 6229, 11739, 11740, 28610 } },
          warlockOnly)
  BOM.Class.SpellDef:scanSpell(spells, 28176, -- TBC: Fel Armor
          { isOwn = true, default = false, singleFamily = { 28176, 28189 } }, -- TBC: Rank 1-2
          { isTBC = true, playerClass = "WARLOCK" })
  BOM.Class.SpellDef:scanSpell(spells, 11735, -- Demon Armor
  ...
  BOM.Class.SpellDef:scanSpell(spells, 696, -- Demon skin
          { isOwn = true, default = false, singleFamily = { 687, 696 } },
          { playerClass = "WARLOCK", maxLevel = 20 })

  BOM.Class.SpellDef:scanSpell(spells, 17953, -- Firestone
          { isOwn          = true, default = false,
            ...
            singleFamily   = { 6366, 17951, 17952, 17953, -- Rank 1-4
                               27250 } }, -- TBC: Rank 5
          warlockOnly)
  BOM.Class.SpellDef:scanSpell(spells, 17728, -- Spellstone
          { isOwn          = true, default = false,
            ...
            singleFamily   = { 2362, 17727, 17728, -- Rank 1-3
                               28172 } }, -- TBC: Rank 4
          warlockOnly)
  BOM.Class.SpellDef:scanSpell(spells, 11730, -- Healtstone
          { isOwn          = true, default = true,
  ...
  BOM.Class.SpellDef:scanSpell(spells, 20757, --Soulstone
          { isOwn          = true, default = true,
            ...
            singleFamily   = { 693, 20752, 20755, 20756, 20757, -- Ranks 1-5
                               27238 } },
          warlockOnly) -- TBC: Rank 6
  BOM.Class.SpellDef:scanSpell(spells, 5500, --Sense Demons
  ...
  -- Pet Management
  ------------------------
  BOM.Class.SpellDef:scanSpell(spells, BOM.SpellId.Warlock.DemonicSacrifice, -- Demonic Sacrifice
          { isOwn = true, default = true, requiresWarlockPet = true },
          warlockOnly)
  BOM.Class.SpellDef:scanSpell(spells, 19028, -- TBC: Soul Link, talent spell 19028, gives buff 25228
          { isOwn              = true, default = true, singleFamily = { 19028, 25228 },
            requiresWarlockPet = true },
          warlockOnly)

  BOM.Class.SpellDef:scanSpell(spells, 688, --Summon Imp
          { type           = "summon", default = true, isOwn = true,
            creatureFamily = "Imp", creatureType = "Demon", sacrificeAuraIds = { 18789 } },
          warlockOnly)

  BOM.Class.SpellDef:scanSpell(spells, 697, --Summon Voidwalker
  ...
  BOM.Class.SpellDef:scanSpell(spells, 712, --Summon Succubus
          { type            = "summon", default = false, isOwn = true,
            reagentRequired = { BOM.ItemId.Warlock.SoulShard },
  ...
  BOM.Class.SpellDef:scanSpell(spells, 30146, --Summon Felguard
          { type            = "summon", default = false, isOwn = true,
            reagentRequired = { BOM.ItemId.Warlock.SoulShard },
...
---@param spells table<string, SpellDef>
---@param enchants table<string, table<number>>
local function bomSetupHunterSpells(spells, enchants)
  local hunterOnly = { playerClass = "HUNTER" }

  BOM.Class.SpellDef:scanSpell(spells, 20906, -- Trueshot Aura
          { isOwn        = true, default = true,
            singleFamily = { 19506, 20905, 20906, -- Ranks 1-3
                             27066 } }, -- TBC: Rank 4
          hunterOnly)

  BOM.Class.SpellDef:scanSpell(spells, 25296, --Aspect of the Hawk
          { type         = "aura", default = true,
            singleFamily = { 13165, 14318, 14319, 14320, 14321, 14322, 25296, -- Rank 1-7
                             27044 } }, -- TBC: Rank 8
          hunterOnly)
  BOM.Class.SpellDef:scanSpell(spells, 13163, --Aspect of the monkey
          { type = "aura", default = false },
          hunterOnly)
  BOM.Class.SpellDef:scanSpell(spells, 34074, -- TBC: Aspect of the Viper
          { type = "aura", default = false },
          { playerClass = "HUNTER", isTBC = true })
  BOM.Class.SpellDef:scanSpell(spells, 20190, --Aspect of the wild
          { type         = "aura", default = false,
            singleFamily = { 20043, 20190, -- Ranks 1-2
                             27045 } }, -- TBC: Rank 3
          hunterOnly)
  BOM.Class.SpellDef:scanSpell(spells, 5118, --Aspect of the Cheetah
          { type = "aura", default = false }, hunterOnly)
  BOM.Class.SpellDef:scanSpell(spells, 13159, --Aspect of the pack
          { type = "aura", default = false }, hunterOnly)
  BOM.Class.SpellDef:scanSpell(spells, 13161, -- Aspect of the beast
          { type = "aura", default = false }, hunterOnly)

  BOM.Class.SpellDef:scanSpell(spells, 1494, -- Track Beast
          { type = "tracking", default = false }, hunterOnly)
  BOM.Class.SpellDef:scanSpell(spells, 19878, -- Track Demon
          { type = "tracking", default = false }, hunterOnly)
  BOM.Class.SpellDef:scanSpell(spells, 19879, -- Track Dragonkin
          { type = "tracking", default = false }, hunterOnly)
  BOM.Class.SpellDef:scanSpell(spells, 19880, -- Track Elemental
          { type = "tracking", default = false }, hunterOnly)
  BOM.Class.SpellDef:scanSpell(spells, 19883, -- Track Humanoids
          { type = "tracking", default = false }, hunterOnly)
  BOM.Class.SpellDef:scanSpell(spells, 19882, -- Track Giants / riesen
          { type = "tracking", default = false }, hunterOnly)
  BOM.Class.SpellDef:scanSpell(spells, 19884, -- Track Undead
          { type = "tracking", default = false }, hunterOnly)
  BOM.Class.SpellDef:scanSpell(spells, 19885, -- Track Hidden / verborgenes
          { type = "tracking", default = false }, hunterOnly)

  -- TODO: Do not use tbc_consumable function, add new flags for pet-buff
  BOM.Class.SpellDef:tbcConsumable(spells, 43771, 33874,
          hunterOnly, "Pet buff +Str",
          { tbcHunterPetBuff = true }) --TBC: Kibler's Bits +20 STR/20 SPI for hunter pet
  BOM.Class.SpellDef:tbcConsumable(spells, 33272, 27656,
...
---@param spells table<string, SpellDef>
---@param enchants table<string, table<number>>
local function bomSetupPaladinSpells(spells, enchants)
  local paladinOnly = { playerClass = "PALADIN" }

  BOM.Class.SpellDef:scanSpell(spells, 25780, --Righteous Fury, same in TBC
          { isOwn = true, default = false })
  ...
  BOM.Class.SpellDef:scanSpell(spells, 25291, --Blessing of Might
  ...
  -- ----------------------------------
  --
  BOM.Class.SpellDef:scanSpell(spells, 10293, -- Devotion Aura
          { type         = "aura", default = false,
            singleFamily = { 465, 10290, 643, 10291, 1032, 10292, 10293, -- Rank 1-7
                             27149 } }, -- TBC: Rank 8
          paladinOnly)
  BOM.Class.SpellDef:scanSpell(spells, 10301, -- Retribution Aura
          { type         = "aura", default = true,
            singleFamily = { 7294, 10298, 10299, 10300, 10301, -- Ranks 1-5
                             27150 } }, -- TBC: Rank 6
          paladinOnly)
  BOM.Class.SpellDef:scanSpell(spells, 19746, --Concentration Aura
          { type = "aura", default = false },
          paladinOnly)
  BOM.Class.SpellDef:scanSpell(spells, 19896, -- Shadow Resistance Aura
          { type = "aura", default = false, singleFamily = { 19876, 19895, 19896, -- Rank 1-3
                                                             27151 } }, -- TBC: Rank 4
          paladinOnly)
  BOM.Class.SpellDef:scanSpell(spells, 19898, -- Frost Resistance Aura
          { type = "aura", default = false, singleFamily = { 19888, 19897, 19898, -- Rank 1-3
                                                             27152 } }, -- TBC: Rank 4
          paladinOnly)
  BOM.Class.SpellDef:scanSpell(spells, 19900, -- Fire Resistance Aura
  ...
  -- ----------------------------------
  --
  BOM.Class.SpellDef:scanSpell(spells, 20773, -- Redemption / Auferstehung
          { type = "resurrection", default = true, singleFamily = { 7328, 10322, 10324, 20772, 20773 } },
          paladinOnly)

  BOM.Class.SpellDef:scanSpell(spells, 20164, -- Sanctity seal
          { type = "seal", default = false },
          { playerClass = "PALADIN", isTBC = false })

  BOM.Class.SpellDef:scanSpell(spells, 5502, -- Sense undead
          { type = "tracking", default = false },
          paladinOnly)

  BOM.Class.SpellDef:scanSpell(spells, 20165, -- Seal of Light
          { type         = "seal", default = false,
            singleFamily = { 20165, 20347, 20348, 20349, -- Ranks 1-4
                             27160 } }, -- TBC: Rank 5
          paladinOnly)
  BOM.Class.SpellDef:scanSpell(spells, 20154, -- Seal of Righteousness
          { type         = "seal", default = false,
            singleFamily = { 20154, 20287, 20288, 20289, 20290, 20291, 20292, 20293, -- Ranks 1-8
                             27155 } }, -- TBC: Seal rank 9
          paladinOnly)
  BOM.Class.SpellDef:scanSpell(spells, 20166, -- Seal of Wisdom
          { type = "seal", default = false },
          paladinOnly)
  BOM.Class.SpellDef:scanSpell(spells, 348704, -- TBC: Seal of Vengeance
          { type         = "seal", default = false,
            singleFamily = { 31801, -- TBC: level 70 spell for Blood Elf
                             348704 } }, -- TBC: Base spell for the alliance races
          paladinOnly)
  BOM.Class.SpellDef:scanSpell(spells, 348700, -- TBC: Seal of the Martyr (Draenei, Dwarf, Human)
          { type = "seal", default = false },
          paladinOnly)
  BOM.Class.SpellDef:scanSpell(spells, 31892, -- TBC: Seal of Blood
          { type         = "seal", default = false,
...
---@param spells table<string, SpellDef>
---@param enchants table<string, table<number>>
local function bomSetupRogueSpells(spells, enchants)
  local duration = tbcOrClassic(DURATION_1H, DURATION_30M) -- TBC: Poisons become 1 hour

  BOM.Class.SpellDef:scanSpell(spells, 25351, --Deadly Poison
  ...
  BOM.Class.SpellDef:scanSpell(spells, 11399, --Mind-numbing Poison
  ...
  BOM.Class.SpellDef:scanSpell(spells, 11340, --Instant Poison
  ...
  BOM.Class.SpellDef:scanSpell(spells, 13227, --Wound Poison
  ...
  BOM.Class.SpellDef:scanSpell(spells, 11202, --Crippling Poison
          { item     = 3776, items = { 3776, 3775 }, isConsumable = true, type = "weapon",
            duration = duration, default = false },
          { playerClass = "ROGUE", minLevel = 20 })
  enchants[11202] = { 603, 22 } --Crippling Poison

  BOM.Class.SpellDef:scanSpell(spells, 26785, --TBC: Anesthetic Poison
...
---@param spells table<string, SpellDef>
---@param enchants table<string, table<number>>
local function bomSetupTrackingSpells(spells, enchants)
...
---@param spells table<string, SpellDef>
---@param enchants table<string, table<number>>
local function bomSetupMiscSpells(spells, enchants)
...
---@param spells table<string, SpellDef>
---@param enchants table<string, table<number>>
local function bomSetupPhysicalDpsBattleElixirs(spells, enchants)
  BOM.Class.SpellDef:scanSpell(spells, 28497, --TBC: Elixir of Major Agility +35 AGI +20 CRIT
          { item          = 22831, isConsumable = true, default = false,
            onlyUsableFor = BOM_PHYSICAL_CLASSES, consumableEra = BOM.TBC_ERA },
          { isTBC = true })
  BOM.Class.SpellDef:scanSpell(spells, 38954, --TBC: Fel Strength Elixir +90AP -10 STA
          { item          = 31679, isConsumable = true, default = false,
            onlyUsableFor = BOM_PHYSICAL_CLASSES, consumableEra = BOM.TBC_ERA },
          { isTBC = true })
  BOM.Class.SpellDef:scanSpell(spells, 28490, --TBC: Elixir of Major Strength +35 STR
          { item          = 22824, isConsumable = true, default = false,
            onlyUsableFor = BOM_PHYSICAL_CLASSES, consumableEra = BOM.TBC_ERA },
          { isTBC = true })
  BOM.Class.SpellDef:scanSpell(spells, 33720, --TBC: Onslaught Elixir +60 AP
...
---@param spells table<string, SpellDef>
---@param enchants table<string, table<number>>
local function bomSetupPhysicalDpsGuardianElixirs(spells, enchants)
end

---ITEMS, applicable to most of the classes, self buffs, containers to open etc
---@param spells table<string, SpellDef>
---@param enchants table<string, table<number>>
local function bomSetupPhysicalDpsConsumables(spells, enchants)
  BOM.Class.SpellDef:scanSpell(spells, 17538, --Elixir of the Mongoose
  ...
  BOM.Class.SpellDef:scanSpell(spells, 11334, --Elixir of Greater Agility
          { item          = 9187, isConsumable = true, default = false,
            onlyUsableFor = BOM_PHYSICAL_CLASSES, consumableEra = BOM.CLASSIC_ERA,
            extraText     = bomClassicHint() })
  BOM.Class.SpellDef:scanSpell(spells, 11405, --Elixir of Giants
          { item          = 9206, isConsumable = true, default = false,
            onlyUsableFor = BOM_PHYSICAL_CLASSES, consumableEra = BOM.CLASSIC_ERA,
            extraText     = bomClassicHint() })
  BOM.Class.SpellDef:scanSpell(spells, 17038, --Winterfall Firewater
          { item          = 12820, isConsumable = true, default = false,
            onlyUsableFor = BOM_PHYSICAL_CLASSES, consumableEra = BOM.CLASSIC_ERA,
            extraText     = bomClassicHint() })
  BOM.Class.SpellDef:scanSpell(spells, 16329, --Juju Might +40AP
          { item          = 12460, isConsumable = true, default = false,
            consumableEra = BOM.CLASSIC_ERA, extraText = bomClassicHint() })
  BOM.Class.SpellDef:scanSpell(spells, 16323, --Juju Power +30Str
  ...
  --
  BOM.Class.SpellDef:scanSpell(spells, 16138, --Sharpening Stone
  ...
  BOM.Class.SpellDef:scanSpell(spells, 28891, --Consecrated Sharpening Stone
  ...
  BOM.Class.SpellDef:scanSpell(spells, 22756, --Elemental Sharpening Stone
  ...
  BOM.Class.SpellDef:scanSpell(spells, 29453, --TBC: Adamantite Sharpening Stone +12 WEAPON +14 CRIT
          ...
          { isTBC = true })
  BOM.Class.SpellDef:scanSpell(spells, 29452, --TBC: Fel Sharpening Stone +12 WEAPON
  ...
  -- Food (pre TBC)
  --
  BOM.Class.SpellDef:scanSpell(spells, 18192, --Grilled Squid +10 Agility
  ...
  BOM.Class.SpellDef:scanSpell(spells, 24799, --Smoked Desert Dumplings +Strength
          { item          = 20452, isConsumable = true, default = false, onlyUsableFor = BOM_PHYSICAL_CLASSES,
            consumableEra = BOM.CLASSIC_ERA },
          { isTBC = false }) -- hide in TBC
  BOM.Class.SpellDef:classicConsumable(spells, 18125, 13810, --Blessed Sunfruit +STR
...
---@param spells table<string, SpellDef>
---@param enchants table<string, table<number>>
local function bomSetupCasterBattleElixirs(spells, enchants)
  -- Not visible in Classic
  BOM.Class.SpellDef:tbcConsumable(spells, 28273, 22710, --TBC: Bloodthistle (Belf only)
          { playerRace = "BloodElf", playerClass = BOM_MANA_CLASSES },
          "+10 spell")

  BOM.Class.SpellDef:tbcConsumable(spells, 28509, 22840) --TBC: Elixir of Major Mageblood +16 mp5
  BOM.Class.SpellDef:tbcConsumable(spells, 28503, 22835, --TBC: Elixir of Major Shadow Power +55 SHADOW
          { playerClass = BOM_SHADOW_CLASSES })
  BOM.Class.SpellDef:tbcConsumable(spells, 28501, 22833, --TBC: Elixir of Major Firepower +55 FIRE
          { playerClass = BOM_FIRE_CLASSES })
  BOM.Class.SpellDef:tbcConsumable(spells, 28493, 22827, --TBC: Elixir of Major Frost Power +55 FROST
          { playerClass = BOM_FROST_CLASSES })
  BOM.Class.SpellDef:tbcConsumable(spells, 28491, 22825, --TBC: Elixir of Healing Power +50 HEAL
          { playerClass = BOM_MANA_CLASSES })
  BOM.Class.SpellDef:tbcConsumable(spells, 33721, 28103, --TBC: Adept's Elixir +24 SPELL, +24 SPELLCRIT
          { playerClass = BOM_MANA_CLASSES })
  BOM.Class.SpellDef:tbcConsumable(spells, 39627, 32067, --TBC: Elixir of Draenic Wisdom +30 SPI
          { playerClass = BOM_MANA_CLASSES })

  -- Visible in TBC but marked as Classic consumables
  BOM.Class.SpellDef:classicConsumable(spells, 24363, 20007, --Mageblood Potion
          { playerClass = BOM_MANA_CLASSES }, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 17539, 13454, --Greater Arcane Elixir
          { playerClass = BOM_MANA_CLASSES }, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 11390, 9155, --Greater Arcane Elixir
          { playerClass = BOM_MANA_CLASSES }, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 11474, 9264, -- Elixir of Shadow Power
          { playerClass = BOM_SHADOW_CLASSES }, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 26276, 21546, --Elixir of Greater Firepower
          { playerClass = BOM_FIRE_CLASSES }, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 21920, 17708, --Elixir of Frost Power
...
local function bomSetupCasterConsumables(spells, enchants)
  ...
  BOM.Class.SpellDef:scanSpell(spells, 25122, -- Wizard Oil
  ...
  BOM.Class.SpellDef:scanSpell(spells, 28898, --Blessed Wizard Oil
...
local function bomSetupBattleElixirs(spells, enchants)
  ...
  BOM.Class.SpellDef:tbcConsumable(spells, 33726, 28104) --TBC: Elixir of Mastery +15 all stats
end
...
local function bomSetupGuardianElixirs(spells, enchants)
  ...
  BOM.Class.SpellDef:tbcConsumable(spells, 39626, 32063) --TBC: Earthen Elixir -20 ALL DMG TAKEN

  BOM.Class.SpellDef:classicConsumable(spells, 3593, 3825, --Elixir of Fortitude
          nil, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 11348, 13445, --Elixir of Superior Defense
          nil, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 24361, 20004, --Major Troll's Blood Potion
          nil, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 11371, 9088, --Gift of Arthas
          nil, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 16326, 12455, --Juju Ember +15FR
          nil, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 16325, 12457, --Juju Chill +15FrostR
          nil, bomClassicHint())

  BOM.Class.SpellDef:classicConsumable(spells, 22730, 18254, --Runn Tum Tuber Surprise
          nil, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 25661, 21023, --Dirge's Kickin' Chimaerok Chops x
          nil, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 22790, 18284, --Kreeg's Stout Beatdown
          { playerClass = BOM_MANA_CLASSES }, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 22789, 18269, --Gordok Green Grog
          nil, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 25804, 21151, --Rumsey Rum Black Label
          nil, bomClassicHint())

  BOM.Class.SpellDef:classicConsumable(spells, 17549, 13461, --Greater Arcane Protection Potion
          nil, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 17543, 13457, --Greater Fire Protection Potion
          nil, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 17544, 13456, --Greater Frost Protection Potion
          nil, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 17546, 13458, --Greater Nature Protection Potion
          nil, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 17548, 13459, --Greater Shadow Protection Potion
          nil, bomClassicHint())

  BOM.Class.SpellDef:tbcConsumable(spells, 28536, 22845) --Major Arcane Protection Potion (crafted and cauldron 32840)
  BOM.Class.SpellDef:tbcConsumable(spells, 28511, 22841) --Major Fire Protection Potion (crafted and cauldron 32846)
  BOM.Class.SpellDef:tbcConsumable(spells, 28512, 22842) --Major Frost Protection Potion (crafted and cauldron 32847)
  BOM.Class.SpellDef:tbcConsumable(spells, 28513, 22844) --Major Nature Protection Potion (crafted and cauldron 32844)
  BOM.Class.SpellDef:tbcConsumable(spells, 28537, 22846) --Major Shadow Protection Potion (crafted and cauldron 32845)
  BOM.Class.SpellDef:tbcConsumable(spells, 28538, 22847) --Major Holy Protection Potion (crafted)
end
...
local function bomSetupItemSpells(spells, enchants)
  ...
  BOM.Class.SpellDef:classicConsumable(spells, 15279, 11567, --Crystal Spire +12 THORNS
          nil, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 15231, 11563, --Crystal Force +30 SPI
          nil, bomClassicHint())
end
...
local function bomSetupFood(spells, enchants)
  ...
  BOM.Class.SpellDef:tbcConsumable(spells, 35254, { 27651, 30155, 27662, 33025 }, --Well Fed +20 STA +20 SPI
          nil, L.TooltipSimilarFoods)
  --BOM.Class.SpellDef:tbc_consumable(spells, 35272, { 27660, 31672, 33026 }) --Well Fed +20 STA +20 SPI

  -- Warp Burger, Grilled Mudfish, ...
  BOM.Class.SpellDef:tbcConsumable(spells, 33261, { 27659, 30358, 27664, 33288, 33293 }, --Well Fed +20 AGI +20 SPI
          { playerClass = BOM_PHYSICAL_CLASSES }, L.TooltipSimilarFoods)
  BOM.Class.SpellDef:tbcConsumable(spells, 43764, 33872, --Spicy Hot Talbuk: Well Fed +20 HITRATING +20 SPI
          { playerClass = BOM_PHYSICAL_CLASSES })
  BOM.Class.SpellDef:tbcConsumable(spells, 33256, { 27658, 30359 }, -- Well Fed +20 STR +20 SPI
          { playerClass = BOM_MELEE_CLASSES }, L.TooltipSimilarFoods)
  BOM.Class.SpellDef:tbcConsumable(spells, 33259, 27655) --Ravager Dog: Well Fed +40 AP +20 SPI
  BOM.Class.SpellDef:tbcConsumable(spells, 46899, 35563, --Charred Bear Kabobs +24 AP
          { playerClass = BOM_PHYSICAL_CLASSES })

  BOM.Class.SpellDef:tbcConsumable(spells, 41030, 32721, --Skyguard Rations: Well Fed +15 STA +15 SPI
          { playerClass = BOM_MANA_CLASSES })
  BOM.Class.SpellDef:tbcConsumable(spells, 33263, { 27657, 31673, 27665, 30361 }, --Well Fed +23 SPELL +20 SPI
          { playerClass = BOM_MANA_CLASSES },
          L.TooltipSimilarFoods)
  BOM.Class.SpellDef:tbcConsumable(spells, 33265, 27663, --Blackened Sporefish: Well Fed +8 MP5 +20 STA
          { playerClass = BOM_MANA_CLASSES })
  BOM.Class.SpellDef:tbcConsumable(spells, 33268, { 27666, 30357 }, --Golden Fish Sticks: Well Fed +44 HEAL +20 SPI
          { playerClass = BOM_MANA_CLASSES },
          L.TooltipSimilarFoods)
end
...
local function bomSetupFlasks(spells, enchants)
  ...
  BOM.Class.SpellDef:classicConsumable(spells, 17628, 13512, --Flask of Supreme Power +70 SPELL
          { playerClass = BOM_MANA_CLASSES }, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 17626, 13510, --Flask of the Titans +400 HP
          nil, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 17627, 13511, --Flask of Distilled Wisdom +65 INT
          { playerClass = BOM_MANA_CLASSES }, bomClassicHint())
  BOM.Class.SpellDef:classicConsumable(spells, 17629, 13513, --Flask of Chromatic Resistance
          nil, bomClassicHint())
...
function BOM.SetupSpells()
  ...
  bomSetupPriestSpells(spells, enchants)
  bomSetupDruidSpells(spells, enchants)
  bomSetupMageSpells(spells, enchants)
  bomSetupShamanSpells(spells, enchants)
  bomSetupWarlockSpells(spells, enchants)
  bomSetupHunterSpells(spells, enchants)
  bomSetupPaladinSpells(spells, enchants)
  bomSetupWarriorSpells(spells, enchants)
  bomSetupRogueSpells(spells, enchants)

  bomSetupTrackingSpells(spells, enchants)
  bomSetupMiscSpells(spells, enchants)
