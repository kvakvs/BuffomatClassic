...
function shamanModule:SetupShamanSpells(spells, enchants)
  ...
  buffDefModule:createAndRegisterBuff(spells, 546, -- Water Walking (no reagent in WotLK with a glyph)
          { default = false, duration = allBuffsModule.DURATION_10M, singleFamily = { 546 } }
  )            :DefaultTargetClasses(allBuffsModule.BOM_ALL_CLASSES)
               :RequirePlayerClass("SHAMAN")
               :Category(allBuffsModule.CLASS)
  buffDefModule:createAndRegisterBuff(spells, 131, -- Water Breathing (no reagent in WotLK with a glyph)
