...
function shamanModule:SetupShamanSpells(allBuffs, enchants)
  ...
  buffDefModule:createAndRegisterBuff(allBuffs, 16342, -- Flametongue Weapon
  ...
  buffDefModule:createAndRegisterBuff(allBuffs, 16356, -- Frostbrand Weapon
          { type         = "weapon", isOwn = true, isConsumable = false,
            default      = false, singleDuration = enchantmentDuration,
  ...
  buffDefModule:createAndRegisterBuff(allBuffs, 16316, -- Rockbiter Weapon (Classic and TBC)
          { type         = "weapon", isOwn = true, isConsumable = false,
            default      = false, singleDuration = enchantmentDuration,
  ...
  buffDefModule:createAndRegisterBuff(allBuffs, 16362, -- Windfury Weapon
          { type         = "weapon", isOwn = true, isConsumable = false,
            default      = false, singleDuration = enchantmentDuration,
  ...
  buffDefModule:createAndRegisterBuff(allBuffs, 51730, -- WotLK: Earthliving Weapon
          { type         = "weapon", isOwn = true, isConsumable = false,
            default      = false, singleDuration = enchantmentDuration,
  ...
  buffDefModule:createAndRegisterBuff(allBuffs, 10432, -- Lightning Shield / Blitzschlagschild
  ...
  buffDefModule:createAndRegisterBuff(allBuffs, 546, -- Water Walking (no reagent in WotLK with a glyph)
          { default = false, duration = allBuffsModule.DURATION_10M, singleFamily = { 546 } }
  )            :DefaultTargetClasses(allBuffsModule.BOM_ALL_CLASSES)
               :RequirePlayerClass("SHAMAN")
               :Category(allBuffsModule.CLASS)
  buffDefModule:createAndRegisterBuff(allBuffs, 131, -- Water Breathing (no reagent in WotLK with a glyph)
          { default = false, duration = allBuffsModule.DURATION_10M, singleFamily = { 131 } }
  )            :DefaultTargetClasses(allBuffsModule.BOM_ALL_CLASSES)
               :RequirePlayerClass("SHAMAN")
               :Category(allBuffsModule.CLASS)

  buffDefModule:createAndRegisterBuff(allBuffs, 33736, -- TBC: Water Shield 1, 2
  ...
  buffDefModule:createAndRegisterBuff(allBuffs, 20777, -- Ancestral Spirit / Auferstehung
